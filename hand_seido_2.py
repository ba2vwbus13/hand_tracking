...
files = glob.glob("/home/ic1/hand_tracking/HS_image/入力/陸上/0.5m/up/*")
...
for f in files:
    ...
    if points is not None:
        ...
        if p[0][0] > p[17][0]:
            ...
            if p[2][1] > p[4][1] and p[6][0] < p[8][0] and p[10][0] < p[12][0] and p[14][0] < p[16][0] and p[18][0] < p[20][0]:
                ...
                print('ちょっと待って！')
                stopkei += 1
            elif p[2][1] > p[4][1] and [3][0] > p[4][0] and p[6][0] < p[8][0] and p[10][1] > p[12][1] and p[14][1] > p[16][1] and p[18][1] > p[20][1]:
                print('OK')
                okkei += 1
                cv2.putText(frame, 'OK', (400,200), cv2.FONT_HERSHEY_SIMPLEX, 2.0, (0, 255, 255), 5)
            elif p[2][0] > p[4][0] and p[6][0] > p[8][0] and p[10][0] > p[12][0] and p[14][0] > p[16][0] and p[18][0] > p[20][0]:
                print('エア切れ')
                airkei += 1
                ...
                print('耳抜き')
                equkei += 1
                cv2.putText(frame, 'equalization', (200,200), cv2.FONT_HERSHEY_SIMPLEX, 2.0, (0, 255, 255), 5)
            elif p[2][0] > p[4][0] and p[6][1] > p[8][1] and p[10][1] < p[12][1] and p[14][1] < p[16][1] and p[18][1] < p[20][1]:
                print('１')
                onekei += 1
                cv2.putText(frame, '1', (400,200), cv2.FONT_HERSHEY_SIMPLEX, 2.0, (0, 255, 255), 5)
                inf = 1
            else :
    ...
    count = 0
    count = '%03d' % maisu
    name = "up_ground_out" + count + ".jpg"
    cv2.imwrite('/home/ic1/hand_tracking/HS画像/出力/陸上/0.5m/up/'+name, frame)
   
    key = cv2.waitKey(1)
    if key == 27:
        break

seido = upkei / maisu * 100
print('総枚数', maisu)
print('upの表示回数', upkei)
print('downの表示回数', downkei)
print('stopの表示回数', stopkei)
print('1の表示回数', onekei)
print('エア切れの表示回数', airkei)
print('問題ありの表示回数', prokei)
print('耳抜きの表示回数', equkei)
print('OKの表示回数', okkei)
print('精度', seido, '％')
print('ErrorList')
for s in errorList:
